...
export class TTour {
	...
	calcTourLength() : void {
		...
		for (let i:number = 0;i<this.fCitiesTour.length;++i) {
			this.fTourLength += this.fTSPMap.getDistance(this.fCitiesTour[i], this.fCitiesTour[this.getCorrectIndex(i+1)]);
		}
	}
	...
	gain(va:number, vb:number, vc:number, vd:number):number {
		return this.fTSPMap.getDistance(va, vb) + this.fTSPMap.getDistance(vc, vd) - this.fTSPMap.getDistance(va, vc) - this.fTSPMap.getDistance(vb, vd);
	}
